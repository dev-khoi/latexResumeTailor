...
const extractHTMLFromUrl = async (url: string): Promise<string> => {
  if (process.env.NEXT_PUBLIC_ENVIRONMNET == "development") {
    try {
  ...
  } else if (process.env.NEXT_PUBLIC_ENVIRONMNET === "deployment") {
    try {
  ...
  throw new Error(
    "NEXT_PUBLIC_ENVIRONMNET environment variable not set properly"
  )
}
